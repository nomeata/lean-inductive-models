/- **The nested occurrence is inside a lambda in a container argument**, a
   ...
   | `Flat.obj : Ctr N (fun _ => Flat) → Flat` | a container with **no recursive field**, so the redex's hypothesis is the *only* entry in the vector | the arm that is not `unpack`: before the fix this one failed at the declaration-local recursor model's first ι theorem, not at `unpack_0` |
