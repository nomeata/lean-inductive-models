...
/-- Kernel structure-like, but declared as an inductive and hence without projection names. -/
inductive EtaBare (alpha : Type u) : Type u where
  | mk (value : alpha) (witness : Eq value value) : EtaBare alpha

/- Independent mutual structure-like members: both have kernel eta. -/
mutual
inductive EtaMutualLeft (alpha : Type u) : Type u where
  | mk (value : alpha) : EtaMutualLeft alpha
inductive EtaMutualRight (alpha : Type u) : Type u where
  | mk (value : alpha) (witness : Eq value value) : EtaMutualRight alpha
end
...
/-- One constructor, but a non-promotable index excludes structure eta. -/
inductive EtaIndexed (alpha : Type u) (step : alpha → alpha) : alpha → Type u where
  | mk (value : alpha) : EtaIndexed alpha step (step value)
...
--#export EtaMutualLeft EtaMutualRight EtaProposition EtaProposition.proof
