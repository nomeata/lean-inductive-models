/- **A declaration every one of whose constructors has a bare recursive field
   is empty, and that — not linearity — is the shape class the empty arm models.**

   The empty arm is the exact model of this shape: the carrier is the derived lift of
   ...
   (`Simple/ArmEmpty.lean:38-70`).  It costs `Nat`, the lift, and no axiom.
   ...
   * `NbBr` has two recursive fields instead of one.  It is just as empty, and
     just as exactly modelled by the lift of `⊥`.  It used to be handed to the
     tree arm, which built a W-type with no leaves: a spliced `_wcore` fragment,
     and `Classical.choice` beside it, for a carrier that is `⊥`.  It is the
     empty arm's now, at 8 declarations against the whole fragment `NbInf` still
     pays for below.
   * `NbInf` recurses under a binder whose domain is **inhabited**.  Still
     empty, and still the tree arm's, because a recursive occurrence under a binder
   ...
   * `NbVac` is the boundary, and the reason the statement is not "no base
     ...
     **inhabited**.  An empty arm generalised to "no base constructor" would model
