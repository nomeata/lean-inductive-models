...
def main : IO UInt32 := do
  ...
  let (shapeDecls, _) ← runFixtureOutput "tests/mutual_shapes.ndjson"
    { noGeneration with mutualModels := true }
  ...
  let (indexDecls, _) ← runFixtureOutput "tests/mutual_index.ndjson"
    { noGeneration with mutualModels := true }
  ...
  let (keyDecls, keyReport) ← runFixtureOutput "tests/mutual_keying.ndjson"
    { noGeneration with mutualModels := true }
  ...
  let privateInput ← readFixture "tests/mutual_nonrec.ndjson"
  let privateInput :=
    { privateInput with
