...
static lean_obj_res lean_inductive_models_io_error(int error, b_lean_obj_arg path) {
    return lean_io_result_mk_error(lean_decode_io_error(error, path));
}

lean_obj_res lean_inductive_models_spool_seek(b_lean_obj_arg handle, uint64_t offset) {
    if (offset > INT64_MAX) {
        return lean_inductive_models_io_error(EINVAL, NULL);
    }
    ...
    if (sizeof(off_t) < sizeof(int64_t)) {
        return lean_inductive_models_io_error(ENOSYS, NULL);
    }
    result = fseeko(file, (off_t)offset, SEEK_SET);
#endif
    if (result != 0) {
        return lean_inductive_models_io_error(errno, NULL);
    }
 ...
 * called.
 */
lean_obj_res lean_inductive_models_spool_mkdir_private_at(
    ...
    if (parent_fd < 0) {
        return lean_inductive_models_io_error(errno, parent_path);
    }
    struct stat status;
    if (fstat(parent_fd, &status) != 0) {
        int error = errno;
        close(parent_fd);
        return lean_inductive_models_io_error(error, parent_path);
    }
    if (status.st_uid != geteuid() || (status.st_mode & (S_IWGRP | S_IWOTH)) != 0) {
        close(parent_fd);
        return lean_inductive_models_io_error(EACCES, parent_path);
    }
    if (mkdirat(parent_fd, leaf, S_IRWXU) != 0) {
        int error = errno;
        close(parent_fd);
        return lean_inductive_models_io_error(error, leaf_name);
    }
    close(parent_fd);
    return lean_io_result_mk_ok(lean_box(0));
#else
