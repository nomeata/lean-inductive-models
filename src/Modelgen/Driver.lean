...
`EDecl.toDeclaration`, this retains the metadata which is not itself an input
to kernel insertion, so `Environment.replay` can compare constructors and
recursors against the values regenerated by Lean. -/
...
def typeCheckExport (x : Export) : MetaM (Except String Unit) := do
  ...
  -- Lean's quotient is one kernel declaration represented by four export
  -- constants.  Replaying any one installs all four; retain the exact records
  -- for the validation above, but submit only the principal name.
  let replayConstants := constants.erase `Quot.mk |>.erase `Quot.lift |>.erase `Quot.ind
  let checked? ← try
      pure (some (← base.replay replayConstants))
    catch error =>
  ...
  for declaration in x.decls do
    if let .induct types constructors recursors := declaration then
      ...
      -- matching Lean's kernel-arena checker.  Safe inductive blocks are fully
      -- present and their exported bookkeeping is compared as well.
      unless types.any (·.isUnsafe) do
