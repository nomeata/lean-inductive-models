...
`modelBefore` stores source-owner roots rather than array indices.  Besides
surviving a reorder, this is the useful incremental representation: the exact
public-slot relation is observed while both owner and model records are live,
then the records may be dropped. -/
...
`modelBefore` records the exact owners whose public model interface this record
contributes to. -/
...
def summaries (x : Export) (prefer : EDecl → Bool := fun _ => false)
  ...
  for family in Check.discover x do
    for model in family.decls do
      let summary := result[model]!
      unless summary.modelBefore.contains family.owner do
        result := result.set! model
          { summary with modelBefore := summary.modelBefore.push family.owner }
  return result
