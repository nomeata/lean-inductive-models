...
def Export.constantInfosLegacyOracle (x : Export) :
...
def typeCheckExportLegacyOracle (x : Export) : MetaM (Except String Unit) := do
  let base ← getEnv
  let constants ← match x.constantInfosLegacyOracle with
    | .error message => return .error message
